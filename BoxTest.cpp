...
void BoxTest::initWorld()//初始化世界
{
    ...
    //是否允许物体休眠 如果允许物体休眠可以提高物理世界的处理效率， 只有发生碰撞时才唤醒对象  节省CPU
    world ->SetAllowSleeping(true);
    //是否开启连续碰撞，计算机智能把一段连续的时间分成许多离散的时间点,在对每一个时间点之间的行为进行机选，如果时间点分割的不过细致，速度较快的两个物体碰撞时可能产生穿透，连续碰撞将启用特殊的算法避免该算法
...
bool BoxTest::onTouchBegan(Touch * touch,Event * event)
{
    ...
    //addNewJoinAtPosition(touch->getLocation());
    return  true;
}
...
void BoxTest::addNewBoxAtPosition(Vec2 p)
{
    ...
    bodyDef.position.Set(p.x/PTM_PATIO, p.y/PTM_PATIO);//设置位置  单位转化为 米
    b2Body * body = world->CreateBody(&bodyDef);//创建物体
    body->SetUserData(sprite);//将精灵放置到物体的userdata属性中 ，这样便于我们从物体中获取相关联的物体
    
    //定义两米见方的盒子形状
    b2PolygonShape dynamicBox;
    dynamicBox.SetAsBox(1.1, 1.1);//盒子左下角为 （0，0），中心点为（1，1） 这样就是两米见方
    ...
    fixtureDef.restitution  = 1.0f;//弹性
...
void BoxTest::addNewCircleAtPosition(Vec2 p)
{
    ...
    bodyDef.type = b2_dynamicBody;//设置物体类型为动态刚体
    ...
    fixtureDef.density = 0.2;//密度
    fixtureDef.restitution = 1.0;//弹性体的复原弹性
    fixtureDef.friction = 0.0;//摩擦力
    
    body->CreateFixture(&fixtureDef);
}
void BoxTest::addNewJoinAtPosition(Vec2 p)//添加一个关节
{
    //创建精灵1
    auto sp1 = Sprite::create("BoxA.png");
    sp1->setPosition(p);
    addChild(sp1);
    //动态物体1定义
    b2BodyDef bodyDef;
    ...
    body1->SetUserData(sp1);
    
    //创建精灵2
    auto sp2 = Sprite::create("BoxB.png");
    sp2->setPosition(p+Vec2(100, 0));
    addChild(sp2);
    
    //动态物体2定义
    bodyDef.type = b2_dynamicBody;
    Vec2 pos2 = sp2->getPosition();
    bodyDef.position.Set(pos2.x/PTM_PATIO, pos2.y/PTM_PATIO);
    
    b2Body *body2 = world->CreateBody(&bodyDef);
    body2->SetUserData(sp2);
    ...
    fixtureDef.restitution = 0.9f;//弹性
    //使用夹具固定形状到物体上
    ...
    joinDef.Initialize(body1, body2, body1->GetWorldCenter(), body2->GetWorldCenter());
    
    joinDef.collideConnected = true;//允许相连的物体碰撞
    body1->GetWorld()->CreateJoint(&joinDef);
    /*
...
void BoxTest::update(float dt)
{
    ...
    for (b2Body *b = world ->GetBodyList(); b; b = b->GetNext()) {
        if (b->GetUserData() != nullptr) {
            ...
            sprite->setRotation(-1*CC_RADIANS_TO_DEGREES(b->GetAngle()));//CC_RADIANS_TO_DEGREES  弧度转化为角度
        }
    }
 }
